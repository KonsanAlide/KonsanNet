...
namespace CXCommunication
{
    ...
    class CXConnectionObject
    {
        ...
        public:
            ...
            //send a packet to the peer
            //dwLen: the data size of the CXPacketBodyData.buf.
            //if failed to send packet, this function will free the buffer pBufObj, the caller not need to free it,
            //pBufObj: input packet buffer, must be allocate from memory cache
            bool  SendPacket(PCXBufferObj pBufObj,DWORD dwLen, DWORD dwMesCode, PCXMessageData pMes = NULL, bool bLockBySelf = false);

            //prepare data and send it
            bool  PreparedAndSendPacket(PCXBufferObj pBufObj);
			...
			int  CallbackTaskProcess(void* pData, int iProcessRet,int iTaskType);
			...
			//get a rpc object attached in this session
			void * GetAttachRPCObject(string strObjectGuid, string strPacketGuid="");

			//create a rpc object to attach to this session
			void*BuildAProxyRPCObject(void *pSession,string strObjectGuid);
			...
			void   PushRPCObj(string strPacketGuid, void* pObj);

			//pop the key-value pair from the map
			void*  PopRPCObj(string strPacketGuid);

            //get the rpc object from the map
            void*  GetRPCObj(string strPacketGuid);

            //push a connection to the internal connection map
            void   PushInternalConnection(string strPacketGuid, uint64 uiIndex);

            //pop the key-value pair from the map
            uint64 PopInternalConnection(string strPacketGuid);

            void*  GetRequestedObject(string strPacketGuid);

        protected:
            //compress and encrypt the data
            bool  PreparedData(PCXBufferObj pInputBuf);

            bool  BuildPacketHeader(PCXBufferObj pBufObj, DWORD dwOrignDataLen);
